...
- `CENTL-Marsa` is the undeletable harbor that ports the Camp stay to
  macOS and Windows. `make marsa-build` and `scripts/marsa-install`
  produce `centl`, `centl-physics`, and `centl-sci` from the committed
  generated core without F*. It is not Oasis. `install` now sends those
  kernels to the harbor instead of a dead end.

#!/usr/bin/env python3
"""Independent finite regression for the hard-prime k=55 forced-2 closure."""
...
import classify_k55_forced2_states as forced2
...
def forced_state_from_factorization(fac: dict[int, int]) -> tuple[int, int]:
    if fac.get(2, 0) < 1:
        raise ValueError("hard-prime C55 is missing the universal factor 2")
...
def analyze(limit: int) -> dict[str, object]:
    ...
    return {
        "analysis": "k55-forced2-structural-regression-v1",
        "limit": limit,

...
`./centl es go` is the infinite hunt. It resumes from `findings/seeds/current.json`, collects letters, and stops when you press Ctrl+C. `--random` begins a new hunt at a random start factor. A finished hunt is not a proof. `strike` stays false until a universal certificate exists.

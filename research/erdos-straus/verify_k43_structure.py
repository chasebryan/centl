#!/usr/bin/env python3
"""Independent finite regression for the exact k=43 state classification.

This generates Mordell-hard primes, factors C=(p+43)/4, builds the divisor
residue set of C^2 directly modulo 43, and compares direct two-target
membership with the separately closed C_42 state classifier.
"""
...
import classify_k43_states as k43
...
def state_from_factorization(fac: dict[int, int]) -> tuple[int, int]:
    ...
    for q, e in fac.items():
        a = k43.LOG[q % MOD]
        for _ in range(e):
            mask, center = k43.transition((mask, center), a)
    return mask, center
...
def analyze(limit: int) -> dict[str, object]:
    ...
    for p in hard:
        ...
        fac = factor(C, trial)
        if MOD in fac:
            mismatch.append({"kind": "nonunit-factor", "p": p, "C": C})
            continue
        ...
        if state not in closed:
            ...
            continue
        predicted_hit = not k43.is_miss(state)
...
def main() -> int:
    ...
    args = ap.parse_args()
    if args.limit < 43:
        raise SystemExit("--limit must be >= 43")

#!/usr/bin/env python3
"""Exact finite-state classifier for the fixed k=59 two-target problem."""
...
EXPECTED = {
    ...
    "miss_states": 61215,
    "pure_qr_states": 4671,
    "pure_qr_miss_states": 4671,
    "legendre59_miss_branches": {"+1": 32110, "-1": 29105},
    "min_nonresidue_histogram": {0: 4671, 1: 19014, 2: 24783, 3: 9808, 4: 2656, 5: 283},
}
...
def min_nonresidue_cost(states: set[tuple[int, int]]):
    """Lexicographically minimize (odd-log valuation units, total units)."""
    start = (1, 0)
    inf = (10**9, 10**9)
    dist = {start: (0, 0)}
...
def target_preserving_scalar_automorphisms() -> list[int]:
    out = []
    for m in range(N):
        if __import__('math').gcd(m, N) != 1:
            continue
        if (m * TYPE_I) % N != TYPE_I:
            continue
        if (m * MINUS_ONE) % N != MINUS_ONE:
            continue
        out.append(m)
    return out
...
def analyze(include_rows: bool) -> dict:
    if TYPE_I_RESIDUE != 44 or TYPE_I != 27 or MINUS_ONE != 29:
        raise SystemExit("k=59 target coordinates changed unexpectedly")
    ...
    out = {
        "analysis": "k59-two-target-state-closure-v1",
        "group": "C58",
        ...
        "target_preserving_scalar_automorphisms": target_preserving_scalar_automorphisms(),
    ...
    if out["target_preserving_scalar_automorphisms"] != [1]:
        raise SystemExit("unexpected nontrivial target-preserving scalar symmetry at k=59")
...
def main() -> int:
    ...
    if args.json:
        ...
        print("k=59 exact two-target state closure")
        for key in ("states", "hit_states", "miss_states", "pure_qr_miss_states"):
            print(f"{key}: {report[key]}")
        print(f"Legendre(59/p) miss branches: {report['legendre59_miss_branches']}")

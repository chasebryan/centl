...
The landed k31 theorem states that a miss occurs exactly when every rational prime factor of D is a nonzero quadratic residue modulo31.

Conditional on a miss, the state has exactly two modes:
...
The residue2 is a quadratic residue modulo31, but
...
In the odd case the factor2 itself supplies a definite QR31 residue outside the BARE stabilizer.

This is stronger than a statistical correlation: the formal product-state combination
...
The local companion cofactors through k55 are
...
If k31 is BARE, then t is even. Therefore the only possible support topology is
...
with the B-L edge equal to2 or4 according to t modulo4.
...
If t is odd and k31 misses, then k31 is necessarily FULL_QR and

```text
gcd(D,J)=2,
```

while B,G,L are pairwise coprime.
...
So the product state

`k31_mode × parity × support_seam`

does not form a Cartesian product.

The exact realizable schematic states are
...
This is the first explicit elimination of a formal product-state combination using two previously independent-looking coordinates.

Before this theorem one could schematically store
...
After the theorem, the state grammar itself should encode the dependency:
...
That is a genuine state transition/reduction rule. No finite census is needed to apply it.
...
The strongest continuation is to find additional non-Cartesian couplings among

`k27_mode × k31_mode × k35_branch × route-conditioned phase × support_seam`.

A particularly promising target is the Route-B k47 normal form, whose THIN/FULL_QR mode may also couple to parity through the factor2 residue. If that theorem lands, the k31 and k47 modes can be coupled on the same Route-B ancestry rather than analyzed independently.

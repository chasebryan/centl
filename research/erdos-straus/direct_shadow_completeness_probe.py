...
A bounded search failure is NOT a counterexample.  Only an explicit witness
is promoted to a theorem certificate.  The purpose of this probe is to try to
kill the conjecture and, if it survives a finite range, freeze exact witnesses
for every candidate in that range.
"""
...
def parameter_constraints(
    ...
    # Put the densest constraints first so witness scans reject quickly.
...
def find_witness(
    ...
    for s in range(limit + 1):
        if any((s % q) in R for _j, q, R in constraints):
            continue
        x0 = r + L * s
        if require_reduced and math.gcd(x0, M) != 1:
            continue
        return s, x0, Q
    return None
...
def main() -> None:
    ...
    ap.add_argument("--k-limit", type=int, default=600)
    ap.add_argument("--search-limit", type=int, default=250000)
    ...
    for k in range(1, args.k_limit + 1):
        for h, t in admissible_pairs(k, traps):
            ...
            iw = find_witness(r, L, constraints, args.search_limit, False)
            if iw is None:
            ...
            rw = find_witness(r, L, constraints, args.search_limit, True)
            rec = {
                "k": k,
                "h": h,
                "t": t,
    ...
    # CENTL receives the hardest reduced witnesses and certifies the two CRT
    # progression identities symbolically.  Number-theoretic avoidance remains
    # independently checked by this probe and the separate verifier.
    centl = [

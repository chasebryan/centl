...
Replays a completed direct-shadow bundle, reconstructs the single-active C1
residual kernels using class_c_census.py, and measures the exact variable-
elimination geometry of residual signatures {11,13} and {3,11,13}.

Finite output is a theorem-certificate for the supplied range only.
"""
...
def coordinate_domains(r: int, L: int, residual: list[dict]) -> tuple[dict[int, int], dict[int, list[int]]]:
...
def triple_forbidden(edge: dict, c3: int, a11: int, b13: int) -> bool:
    fac = factor_dict(edge["q"])
    coords = {3: c3, 11: a11, 13: b13}
    return any(
        all(coords[p] % (p**a) == v % (p**a) for p, a in fac.items())
        for v in edge["R"]
    )
    ...
    for a in D11:
        ...
        for b0 in base13:
            if not any(pair_forbidden(e, a, b0) for e in coarse):
                coarse_safe_base.append(b0)
        min_base13 = min(min_base13, len(coarse_safe_base))

        full_safe_coarse = [
            b for b in D13 if not any(pair_forbidden(e, a, b) for e in coarse)
        ]
        full_safe_all = [
            b for b in full_safe_coarse if not any(pair_forbidden(e, a, b) for e in fine13)
        ]
...
def analyse_31113(r: int, L: int, residual: list[dict]) -> dict:
    ...
    three_edges = [e for e in residual if 3 in factor_dict(e["q"]) and len(factor_dict(e["q"])) > 1]
    ...
    # Stronger diagnostic: these bounds are checked on every unary-safe base
    # pair, before 11/13 mixed rows are even imposed.
    for a0 in base11:
        for b0 in base13:
            f3: set[int] = set()
            f9: set[int] = set()
            ...
            for edge in three_edges:
                ...
                selected = []
                for v in edge["R"]:
                    if 11 in fac and v % (11 ** fac[11]) != a0 % (11 ** fac[11]):
                        continue
                    if 13 in fac and v % (13 ** fac[13]) != b0 % (13 ** fac[13]):
                        continue
    ...
    # Exact full extension count after all pair rows and every 3-row.
    ...
    for a in D11:
        safe_b = [
            b for b in D13 if not any(pair_forbidden(e, a, b) for e in pair_edges)
        ]
        min_pair_extensions = min(min_pair_extensions, len(safe_b))
        for b in safe_b:
            count = 0
            for c in D3:
                if not any(triple_forbidden(e, c, a, b) for e in three_edges):
                    count += 1
            safe3_counts[count] += 1
            min_safe3 = min(min_safe3, count)

    pair["minimum_safe_full13_values_per_full11_all_pair_rows"] = min_pair_extensions
    pair["fine13_new_hits_after_coarse_pair_stage"] = sum(
        1
        for a in D11
        for b in D13
        if not any(pair_forbidden(e, a, b) for e in coarse_pair)
        and any(pair_forbidden(e, a, b) for e in fine_pair)
    )
    ...
    return {
        ...
        "safe_full3_count_histogram": {str(k): v for k, v in sorted(safe3_counts.items())},
    }
...
def main() -> None:
    ...
    profiles = {k: cc.layer_profile(k, mfac) for k in {int(w["k"]) for w in witnesses}}
    ...
    for index, rec in enumerate(witnesses, start=1):
        ...
        if primes == (11, 13):
            cert1113.append({**common, "conditioned": analyse_1113(r, L, residual)})
        elif primes == (3, 11, 13):
            cert31113.append({**common, "conditioned": analyse_31113(r, L, residual)})
    ...
    result = {
        ...
        "signature_1113": {
            "minimum_safe_base13_classes_per_full11": min(
                x["conditioned"]["minimum_safe_base13_classes_per_full11"] for x in cert1113
            ),
            "minimum_safe_full13_values_per_full11": min(
                x["conditioned"]["minimum_safe_full13_values_per_full11"] for x in cert1113
            ),
            "fine13_new_hits_after_coarse_pair_stage": sum(
                x["conditioned"]["fine13_new_hits_after_coarse_pair_stage"] for x in cert1113
            ),
        },
        "signature_31113": {
            "minimum_unary_full11_domain": min(x["conditioned"]["unary_full_domain_sizes"]["11"] for x in cert31113),
            "minimum_unary_base11_classes": min(x["conditioned"]["pair_stage"]["unary_base_domain_sizes"]["11"] for x in cert31113),
            "minimum_unary_base13_classes": min(x["conditioned"]["pair_stage"]["unary_base_domain_sizes"]["13"] for x in cert31113),
            "minimum_safe_base13_classes_per_full11": min(x["conditioned"]["pair_stage"]["minimum_safe_base13_classes_per_full11"] for x in cert31113),
            "minimum_safe_full13_values_per_full11": min(x["conditioned"]["pair_stage"]["minimum_safe_full13_values_per_full11_all_pair_rows"] for x in cert31113),
            "fine13_new_hits_after_coarse_pair_stage": sum(x["conditioned"]["pair_stage"]["fine13_new_hits_after_coarse_pair_stage"] for x in cert31113),
            "maximum_collective_forbidden_mod3_classes": max(x["conditioned"]["maximum_collective_forbidden_mod3_classes"] for x in cert31113),
            "maximum_collective_forbidden_mod9_classes": max(x["conditioned"]["maximum_collective_forbidden_mod9_classes"] for x in cert31113),
            "fine81_new_hits_after_mod9_stage": sum(x["conditioned"]["fine81_new_hits_after_mod9_stage"] for x in cert31113),
            "minimum_safe_full3_values_per_safe_1113_pair": min(x["conditioned"]["minimum_safe_full3_values_per_safe_1113_pair"] for x in cert31113),
        ...
        "claim_boundary": "Exact only for the supplied finite bundle; universal conditioned-fiber positivity remains open.",
    }

    (args.out / "c1-conditioned-fiber.json").write_text(json.dumps(result, indent=2, sort_keys=True) + "\n")

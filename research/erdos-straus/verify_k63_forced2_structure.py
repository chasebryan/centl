#!/usr/bin/env python3
"""Independent finite regression for the hard-prime k=63 forced-2 closure."""
...
import classify_k63_forced2_states as forced2
...
def forced_state_from_factorization(fac: dict[int, int]) -> tuple[int, int]:
    if fac.get(2, 0) < 1:
        raise ValueError("hard-prime C63 is missing the universal factor 2")
    state = forced2.forced_start()
    remaining = dict(fac)
    remaining[2] -= 1
    if remaining[2] == 0:
        del remaining[2]
    for q, e in remaining.items():
        residue = q % MOD
        if residue not in core.RESIDUE_TO_COORD:
            raise ValueError(f"non-unit factor residue {residue} mod 63")
        g = core.RESIDUE_TO_COORD[residue]
        for _ in range(e):
            state = core.transition(state, g)
    return state
...
def analyze(limit: int) -> dict[str, object]:
    ...
    for p in hard:
        ...
        if state not in closed:
            ...
            continue
        if not core.admissible_center(state[1]):
    ...
    return {
        "analysis": "k63-forced2-structural-regression-v1",
        "limit": limit,
        ...
        "claim": "finite independent regression of a range-free hard-prime forced-2 closure",
    }

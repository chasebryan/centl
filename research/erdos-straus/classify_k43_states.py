#!/usr/bin/env python3
"""Exact finite-state classifier for the fixed k=43 two-target problem.

Use primitive root 3 modulo 43, so the unit group is C_42.  A state is
(D,c), where D is the divisor-log set of C^2 and c=log_3(C).  One
prime-valuation occurrence of log a gives the exact transition
D -> D + {0,a,2a}, c -> c+a.
"""
...
ALL = (1 << N) - 1
PRIMITIVE_ROOT = 3
TYPE_I_RESIDUE = (-pow(4, -1, 43)) % 43
LOG = {pow(PRIMITIVE_ROOT, a, 43): a for a in range(N)}
...
EXPECTED = {
    ...
    "miss_states": 7740,
    "pure_qr_states": 852,
    "pure_qr_miss_states": 852,
    "miss_symmetry_orbits": 4045,
    "miss_symmetry_fixed": 350,
    "miss_symmetry_pairs": 3695,
    "legendre43_miss_branches": {"+1": 4268, "-1": 3472},
    "min_nonresidue_histogram": {0: 852, 1: 2474, 2: 3177, 3: 998, 4: 239},
}
...
def symmetry_elem(a: int) -> int:
    """Nontrivial target-preserving automorphism a -> 29a mod 42."""
    return (29 * a) % N
...
def min_nonresidue_cost(states: set[tuple[int, int]]):
    """Minimize (odd-log valuation units, total valuation units)."""
    start = (1, 0)
    inf = (10**9, 10**9)
    dist = {start: (0, 0)}
...
def analyze(include_rows: bool) -> dict:
    ...
    out = {
        "analysis": "k43-two-target-state-closure-v1",
        "group": "C42",
        ...
        "legendre43_miss_branches": dict(sorted(legendre.items())),
    ...
    if include_rows:
        out["miss_rows"] = [
            {
                "center_log": center,
                "center_residue": pow(PRIMITIVE_ROOT, center, 43),
                "legendre43": "+1" if center % 2 == 0 else "-1",
...
def main() -> int:
    ...
    if args.json:
        ...
        print("k=43 exact two-target state closure")
        for key in ("states", "hit_states", "miss_states", "pure_qr_miss_states", "miss_symmetry_orbits"):
            print(f"{key}: {report[key]}")
        print(f"Legendre(43/p) miss branches: {report['legendre43_miss_branches']}")

...
**Letter targeting (cbap.kernel):** [`cbap.kernel`](cbap.kernel/README.md) is the C engine that hunts LETTER only. Channels A/B/C acquire, lock, and track; channel D verdicts. `./centl es cbap` starts at 0 and resumes. GREAT is not stored.

**Public hunt (2026-08-15):** [`ES-HUNT.md`](ES-HUNT.md) is the operator-facing record. [`bb.kernel`](bb.kernel) (also `B-BervigES.kernel`) is the Python reference; [`CC.kernel`](CC.kernel) is the fast hard-prime engine. Findings live in [`findings/`](findings/START-HERE.md). GREAT/GOOD records are compact JSONL and are not in a git clone (`./centl es fetch`). `--letters-only` files only LETTERS. Letter numbers are content-addressed and identical on every machine that finds the same letter. From the repo root: `./centl es go` (or `./centl es` for the menu). `--from N` and `--random` start another hunt without replacing the first. `./centl es hunts` lists them. Erdős–Straus remains open.

#!/usr/bin/env python3
"""Independent finite regression for the exact k=63 state classification."""
...
import classify_k63_states as k63
...
def state_from_factorization(fac: dict[int, int]) -> tuple[int, int]:
    ...
    for q, e in fac.items():
        residue = q % MOD
        if residue not in k63.RESIDUE_TO_COORD:
            raise ValueError(f"non-unit factor residue {residue} mod 63")
        g = k63.RESIDUE_TO_COORD[residue]
        for _ in range(e):
            state = k63.transition(state, g)
    return state
...
def analyze(limit: int) -> dict[str, object]:
    ...
    for p in hard:
        ...
        if state not in closed:
            ...
            continue
        if not k63.admissible_center(state[1]):
            mismatch.append({"kind": "hard-center-outside-H", "p": p, "center": state[1]})
            continue
        predicted_hit = not k63.is_miss(state)
        ...
        if pow(p % 7, 3, 7) != 1:
            mismatch.append({"kind": "unexpected-legendre7", "p": p})

        outcomes["hit" if direct_hit else "miss"] += 1

    return {
        "analysis": "k63-structural-regression-v1",
        "limit": limit,
...
def main() -> int:
    ...
    args = ap.parse_args()
    if args.limit < MOD:
        raise SystemExit("--limit must be >= 63")

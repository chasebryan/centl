...
class HubClient:
    ...
    def _initial_url(self, path: str) -> str:
    ...
    def download(self, path: str, output: Path) -> None:
        # Reference Qwen preservation is public. File downloads deliberately do
        # not forward HF_TOKEN through hub/CDN redirects; the token is used only
        # for API metadata/rate limits and is never exposed to another host.
        url = self._initial_url(path)
        request = urllib.request.Request(
            url, headers={"User-Agent": "centl-fcf-model-preservation/1"}
        )
        ...
        try:
            try:
                ...
                detail = error.read().decode("utf-8", errors="replace")[:1000]
                die(f"model file HTTP {error.code} for {url}: {detail}")
            except urllib.error.URLError as error:
                die(f"model file request failed for {url}: {error.reason}")
            self.requests += 1
...
def exact_metadata(
    client: HubClient, repository: str, revision: str
    ...
    metadata = client.json(f"/api/models/{encoded}/revision/{resolved}?blobs=true")
...
def sibling_identity(
    ...
    if lfs is not None:
        ...
        identity = lfs.get("sha256")
        if identity is None:
            identity = lfs.get("oid")
        ...
        if not isinstance(identity, str) or not HEX64.fullmatch(identity):
            die("model hub LFS metadata has no valid SHA-256 identity")
        lfs_sha = identity
...
def build_plan(metadata: dict[str, Any]) -> tuple[list[dict[str, Any]], int | None]:
...
def verify_download(path: Path, row: dict[str, Any]) -> dict[str, Any]:
    ...
    return {**row, "bytes": size, "fcf_sha256": local_sha}
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="preserve an immutable model repository revision"
    )
    ...
    parser.add_argument(
        "--plan", action="store_true", help="resolve/list files without downloading"
    )
    ...
    # A mutating capture refuses to use a corrupted finalized mirror before it
    # contacts the model hub. Plan mode is non-mutating and may run independently.
    if not args.plan and (mirror / "MIRROR-SHA256SUMS").exists():
        subprocess.run(
    ...
    rows, known_total = build_plan(metadata)
    ...
    if args.plan:
        for row in rows:
            ...
            print(
                f"file={row['path']} identity={identity} size={row['upstream_size']}"
            )
        return 0

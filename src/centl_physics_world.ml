...
let sphere_world spheres =
  let particle_count = List.length spheres in
  let pair_count = particle_count * (particle_count - 1) / 2 in
  if pair_count > max_contact_pairs then
    raise
      (Physics_error
         (Printf.sprintf "sphere world exceeds the %d-contact-pair safety limit"
            max_contact_pairs));

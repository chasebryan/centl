...
type limits = {
  ...
  factorization : Centl_polynomial_factorization.limits;
...
let default_limits =
  {
    ...
    factorization = Centl_polynomial_factorization.default_limits;
...
let provenance method_ classification =
  `Assoc
    [
      ...
      ("backend", `String "centl-exact-polynomial-factorization");
    ]
...
let error_code = function
  ...
  | Internal_division_error _ | Internal_factorization_error _ -> "internal_error"
...
let enforce_result_limit limits ~method_ response =
  if String.length (Yojson.Safe.to_string response) > limits.max_result_bytes then
    failure ~method_ "resource_limit"
      "polynomial factorization result exceeds the byte limit"
  else response
...
let factorization_json
    (factorization : Centl_polynomial_factorization.factorization) =
  `Assoc
    [
      ("kind", `String "polynomial_rational_factorization");
      ("exact", `Bool true);
      ( "unit",
        Centl_multivariate_polynomial_protocol.rational_json factorization.unit );
      ("factor_semantics", `String "irreducible_factors_over_Q");
      ("irreducible_factorization", `Bool true);
      ("complete", `Bool true);
...
let capabilities limits =
  `Assoc
    [
      ("kind", `String "polynomial_rational_factorization_capabilities");
      ("exact", `Bool true);
      ("univariate", `Bool true);
      ("explicit_variable", `Bool true);
      ("coefficient_domain", `String "Q");
      ("characteristic_zero", `Bool true);
      ("monic_factors", `Bool true);
      ("irreducible_factorization", `Bool true);
      ("multiplicities", `Bool true);
      ("deterministic_ordering", `Bool true);
      ("exact_reconstruction_verified", `Bool true);
      ...
      ( "text",
        `String
          "Complete exact irreducible factorization over explicit univariate Q[x], with canonical monic factors, multiplicities, deterministic ordering, and exact reconstruction verification." );
    ]

let factorize_action ?(cancelled = never_cancelled) limits fields =
  let method_ = "factorize" in
  match
  ...
  | Ok () ->
      ...
      | Ok variable, Some polynomial ->
          begin match parse_polynomial limits "polynomial" polynomial with
          | Error message -> failure ~method_ "invalid_request" message
          | Ok polynomial ->
              begin match
                factorize ~limits:limits.factorization ~cancelled ~variable polynomial
              with
              | Error error ->
...
let dispatch ?(cancelled = never_cancelled) limits action fields =
  if cancelled () && not (String.equal action "capabilities") then
    failure ~method_:action "cancelled" "polynomial factorization was cancelled"
  else
    match action with
    | "capabilities" ->
    ...
    | "factorize" -> factorize_action ~cancelled limits fields
    | _ ->
        failure ~method_:action "invalid_request"
          ("unknown polynomial factorization action " ^ action)

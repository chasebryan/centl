...
let entities_to_yojson (conversion : entity_conversion) =
  `Assoc
    ([
       ("version", `Int 1);
...
let moles_to_yojson (conversion : mole_conversion) =
  `Assoc
    ([
       ("version", `Int 1);
...
let stoichiometry_to_yojson (conversion : stoichiometric_amount) =
  `Assoc
    ([
       ("version", `Int 1);
...
let stoichiometry_request ~source_class ~reaction_text

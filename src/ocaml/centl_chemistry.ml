...
let reaction_elements (reaction : reaction) =
  let add_species set (species : species) =
...
let side_total (species : species list) coefficients element =
  try
    List.fold_left2
...
let conservation_for (reaction : reaction) reactant_coefficients

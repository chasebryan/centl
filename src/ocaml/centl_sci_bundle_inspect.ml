type assurance_count = { label : string; count : int }
...
let assurance_counts extensions =
  extensions
  |> List.fold_left
       (fun counts extension ->
         increment extension.Centl_sci_extensions.assurance counts)
       []
  ...
  if path = "" then Error "bundle path must not be empty"
    ...
    | Ok () ->
        let bundle_workspace =
          Centl_sci_workspace.make ~name:"inspect-bundle" path
        in
...
let render_assurance item = Printf.sprintf "    - %s: %d" item.label item.count
...
let render bundle =
  String.concat "\n"
    ...
    @ [
        ...
        "  mutation: none";
        "  assurance note: bundle inspection preserves recorded assurance and \
         does not promote it.";
      ])
      ...
      ( "enabled_native_extensions",
        `List
          (List.map
             ...
             bundle.enabled_native_extensions) );
      ("disabled_extensions", `Int bundle.disabled_extensions);
      ("packages", `Int bundle.packages);
      ( "assurance_counts",
        `List (List.map assurance_json bundle.assurance_counts) );
      ("mutation", `Bool false);

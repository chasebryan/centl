...
let entry_of_json json =
  match
    ( assoc "revision" json,
      ...
      assoc "scope" json )
  with
  | Some (`Int revision), Some (`Float timestamp_unix), Some (`String actor),
    Some (`String scope) ->
      Some { revision; timestamp_unix; actor; scope }
  | Some (`Int revision), Some (`Int timestamp_unix), Some (`String actor),
    Some (`String scope) ->
      Some
        {
          revision;
          timestamp_unix = float_of_int timestamp_unix;
          actor;
          scope;
        }
  | _ -> None
...
let append_bounded entries entry =
  ...
  if List.length entries <= max_entries then entries
  else List.tl entries
...
let render_entry entry =
  Printf.sprintf "  - r%d — %.6f — %s — %s" entry.revision
    entry.timestamp_unix entry.actor entry.scope
...
let render history =
  String.concat "\n"
    ([
       ...
       "  bounded view: last " ^ string_of_int max_entries ^ " parsed entries maximum";
     ]
    @
    ...
    (if history.truncated then
       [ "  note: older revision events exist but are omitted from this bounded view" ]
     else []))

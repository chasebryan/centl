...
let extension_entry workspace (manifest : Centl_sci_extensions.manifest) =
  match
    Centl_sci_validate.validate workspace manifest.Centl_sci_extensions.name
  with
  | Error message ->
      {
        name = manifest.name;
...
let package_entry workspace (package : Centl_sci_package.t) =
  ...
  | Ok validation ->
      let missing_members =
        validation.members
        |> List.filter (fun (member : Centl_sci_package.member_state) ->
            not member.present)
        |> List.map (fun (member : Centl_sci_package.member_state) ->
            member.name)
      in
      {
        name = package.name;
...
let collect workspace =
  ...
  let packages =
    Centl_sci_package.list workspace |> List.map (package_entry workspace)
  in
      ...
      |> List.concat_map (fun (extension : extension_entry) ->
          let structural =
            if extension.structurally_valid then []
              ...
              [
                "extension " ^ extension.name ^ " failed structural validation";
              ]
          in
          let activation =
            if extension.enabled && extension.kind <> "native_centl" then
              [
                Printf.sprintf
                  "extension %s is enabled with non-native kind %s; it must \
                   not enter the native CENTL loader"
  ...
  let package_lines =
    ...
    |> List.map (fun (package : package_entry) ->
        Printf.sprintf "  - %s — %s — members=%d" package.name
          (if package.valid then "valid" else "invalid")
          package.members)
  in
  String.concat "\n"
    ([
       ...
       ("  health: "
       ^ if healthy report then "healthy" else "attention_required");
    ...
    @ [
        "Assurance note: workspace health is structural consistency, not a \
         trust score.";
      ])
  ...
  let package_json (package : package_entry) =
    `Assoc
      [
        ...
        ( "missing_members",
          `List
            (List.map (fun member -> `String member) package.missing_members) );
      ]
  in
  `Assoc
    [

type dependency = Local_extension of string | External of string | Opaque of string
...
let opaque_names manifest =
  classified_dependencies manifest
  |> List.filter_map (function Opaque name when name <> "" -> Some name | _ -> None)
...
let detect_cycles local_edges =
  ...
  let rec suffix_from name = function
    | [] -> []
    | head :: tail -> if head = name then head :: tail else suffix_from name tail
  in
  let rec visit name =
    ...
    | Some `Visiting ->
        ...
        if suffix <> [] then cycles := canonical_cycle (suffix @ [ name ]) :: !cycles
    | None ->
        Hashtbl.replace state name `Visiting;
        stack := name :: !stack;
        let dependencies = Option.value ~default:[] (List.assoc_opt name edge_map) in
        List.iter visit dependencies;
        begin match !stack with _ :: rest -> stack := rest | [] -> () end;
        Hashtbl.replace state name `Done
  in
  List.iter (fun (name, _) -> visit name) local_edges;
  !cycles
  |> List.sort_uniq (fun left right -> String.compare (cycle_key left) (cycle_key right))
...
let validate workspace =
  ...
  in
  let issues =
    List.concat_map (local_issues manifests) manifest_values @ detect_cycles local_edges
  in
  let invalid =
    List.exists
      (function Missing_extension _ | Cycle _ -> true | Inactive_extension _ -> false)
      issues
  in
  { valid = not invalid; issues; local_edges; external_dependencies; opaque_dependencies }

let issue_text = function
  | Missing_extension { extension; dependency } ->
      Printf.sprintf "%s declares missing local extension dependency %s" extension dependency
  | Inactive_extension { extension; dependency } ->
      Printf.sprintf "%s is enabled but local extension dependency %s is disabled" extension dependency
  | Cycle cycle -> "local extension dependency cycle: " ^ String.concat " -> " cycle
...
let render report =
  String.concat "\n"
    ([
       ...
       "  local dependency edges: " ^ string_of_int (List.length report.local_edges);
    ...
    @ [
        ...
        "  - external:NAME records an external dependency without pretending Caramels validated it";
        "  - unprefixed legacy dependency strings are preserved as opaque provenance";
        "Assurance note: dependency validity is structural composition evidence, not verified-core assurance.";
      ])
...
let to_json report =
  ...
  `Assoc
    [
      ...
      ("issues", `List (List.map (fun issue -> `String (issue_text issue)) report.issues));
      ("local_edges", `List (List.map edge_json report.local_edges));
      ("external_dependencies", `List (List.map edge_json report.external_dependencies));
      ("opaque_dependencies", `List (List.map edge_json report.opaque_dependencies));

...
let invoke ?(max_request_bytes = default_max_request_bytes)
  ...
  if request_bytes > max_request_bytes then Error (Request_too_large request_bytes)
    ...
    try
      ...
      if response_bytes > max_response_bytes then begin
        ...
        begin match status, parsed with
    ...
    | End_of_file -> Error (Protocol_error "adapter closed stdout before one JSONL response")

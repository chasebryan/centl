...
let scaffold workspace ~kind ~name ~target ~summary =
  if not (Centl_sci_workspace.valid_extension_name name) then
    ...
    let root =
      Filename.concat workspace.Centl_sci_workspace.generated
        ("scaffolds/" ^ name)
    in
          ...
          ( "adapter.py",
            ...
            String.concat "\n"
              [
                ...
                "";
                "Contract: JSONL over stdin/stdout. This file is intentionally \
                 inactive";
                "until the user implements and validates the named external \
                 boundary.";
                "\"\"\"";
                "";
                "import json";
            ...
            String.concat "\n"
              [
                "#!/usr/bin/env python3";
                "\"\"\"Structural validation placeholder for the generated \
                 adapter.\"\"\"";
                "";
          ...
          ( "extension.c",
            ...
            String.concat "\n"
              [
                ...
                " * This is intentionally not wired into CENTL's \
                 verified/native loader.";
    ...
    try
      ...
      in
      write_text_file
        (Filename.concat root "scaffold.json")
        (Yojson.Safe.pretty_to_string contract ^ "\n");
      write_text_file
        (Filename.concat root "README.md")
        (String.concat "\n"
           [
             "# " ^ name;
             "";
             ...
             "";
             "This scaffold is intentionally inactive until its adapter/native \
              boundary is implemented and validated.";
             "Generation grants no network or filesystem privileges and does \
              not change verified CENTL core assurance.";
             "External/generated results must remain visibly distinguished \
              from verified-core computation.";
             "";
           ]);
      begin match
        Centl_sci_workspace.write_manifest_detailed workspace ~name
          ~enabled:false ~assurance
          ~source:("generated/scaffolds/" ^ name ^ "/scaffold.json")
    ...
    let body =
      String.concat "\n"
        ...
        @ [
            ...
            "- run the quality gates relevant to the touched implementation \
             layers";

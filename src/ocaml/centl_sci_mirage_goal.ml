...
type gap = {
  cell_id : int;
  status : gap_status;
  capability_matches : string list;
  reason : string;
}
...
let polarity cell =
  if String.uppercase_ascii cell.kind = "NON_GOAL" then Negative
    ...
    let text = lower cell.text in
    if List.exists (fun phrase -> contains phrase text) negative_phrases then Negative
    else Positive
...
let hard_cell cell =
  ...
  | "DIRECTIVE" | "INVARIANT" | "NON_GOAL" -> true
  | _ -> false

let likely_conflict left right =
  hard_cell left
  && hard_cell right
...
let is_alias_request text =
  ...
  List.exists (fun token -> contains token text)
...
let gap_for_cell workspace conflict_pairs cell =
...
let nearest_objective cells before_id =
  cells
  |> List.filter (fun cell ->
         ...
         | _ -> false)
  |> List.rev |> List.hd_opt

let cell_edges cells =
  cells
...
let conflict_edges pairs =
  List.map
    (fun (left, right) ->
      {
        source = cell_node_id left;
        target = cell_node_id right;
        kind = Conflicts_with;
      })
    pairs

let capability_edges workspace cells =
  cells
  |> List.filter (fun cell ->
         match String.uppercase_ascii cell.kind with
         | "DIRECTIVE" | "INVARIANT" -> true
  ...
  |> List.concat_map (fun cell ->
         ...
         |> List.map (fun capability ->
                {
                  source = cell_node_id cell.id;
                  target = capability_node_id capability.name;
...
let cell_nodes cells =
  List.map
    (fun cell ->
      {
        id = cell_node_id cell.id;
        kind = cell_node_kind cell.kind;
        label = cell.text;
        source_cell = Some cell.id;
      })
    cells

let capability_nodes workspace cells =
  cells
  |> List.concat_map (fun cell -> capability_matches workspace cell.text)
  |> List.sort_uniq (fun left right -> String.compare left.Centl_sci_capabilities.name right.name)
  |> List.map (fun capability ->
         {
           id = capability_node_id capability.name;
           kind = Capability;
           label = Centl_sci_capabilities.render capability;
           source_cell = None;
         })

let build workspace cells =
...
let gap_to_json gap =
  `Assoc
    [
      ("cell_id", `Int gap.cell_id);
      ("status", `String (gap_status_text gap.status));
      ( "capability_matches",
        `List (List.map (fun value -> `String value) gap.capability_matches) );
...
let to_json graph =
  `Assoc
    [
      ...
      ("graph_kind", `String "goal_and_capability_gap_graph");
      ("nodes", `List (List.map node_to_json graph.nodes));
      ("edges", `List (List.map edge_to_json graph.edges));
      ("gaps", `List (List.map gap_to_json graph.gaps));
      ( "conflicts",
        `List
          (List.map
             (fun (left, right) -> `List [ `Int left; `Int right ])
             graph.conflicts) );
    ]
...
let analyze workspace spec_path =
...
let render_gap gap =
  let matches =
    match gap.capability_matches with
    | [] -> "none"
...
let render graph =
  let lines =
    [
      "CENTL-MIRAGE goal graph";
      "nodes: " ^ string_of_int (List.length graph.nodes);
      "edges: " ^ string_of_int (List.length graph.edges);
      "conflicts: " ^ string_of_int (List.length graph.conflicts);
      "gaps:";
    ]
    @
    match graph.gaps with
    | [] -> [ "  none" ]
    | values -> List.map (fun gap -> "  " ^ render_gap gap) values
  in
  String.concat "\n" lines

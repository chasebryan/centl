...
Planning does not mutate the FCF mirror.
...
An optional `HF_TOKEN` may be supplied for model-hub authentication/rate limits.
The token is used only as an HTTP Authorization header and is not written to the
snapshot.
...
- large LFS-backed files may have an LFS SHA-256 OID;
...
`SOURCE.json` records the repository identity/source URL, requested and resolved
revisions, capture time, model-card license field when published, file count,
total preserved byte count, and whether authentication was used. It never stores
the token itself.

...
let test_cell_classification () =
  ...
  Alcotest.(check bool) "directive detected" true
    (List.mem Centl_sci_mirage.Directive kinds);
  Alcotest.(check bool) "invariant detected" true
    (List.mem Centl_sci_mirage.Invariant kinds);
  Alcotest.(check bool) "acceptance detected" true
    (List.mem Centl_sci_mirage.Acceptance kinds);
  Alcotest.(check bool) "example detected" true
    (List.mem Centl_sci_mirage.Example kinds);
  Alcotest.(check bool) "non-goal detected" true
    (List.mem Centl_sci_mirage.Non_goal kinds);
  Alcotest.(check bool) "question detected" true
...
let test_ingestion_creates_local_cycle () =
  ...
  Fun.protect
    ...
    (fun () ->
      ...
      | Ok result ->
          Alcotest.(check bool) "stored source exists" true
            (Sys.file_exists result.stored_path);
          Alcotest.(check bool) "specification IR exists" true
            (Sys.file_exists result.spec_path);
          Alcotest.(check bool) "development plan exists" true
            (Sys.file_exists result.plan_path);
          Alcotest.(check bool) "active cycle exists" true
            (Sys.file_exists result.active_path);
          Alcotest.(check bool) "cells extracted" true (result.cell_count >= 4);
          Alcotest.(check bool) "objectives extracted" true
          ...
          | `Assoc fields ->
              ...
              | Some (`String digest) ->
                  Alcotest.(check string) "digest is stable result identity"
...
let test_document_size_limit () =
  ...
  Fun.protect
    ...
    (fun () ->
      ...
      Fun.protect
        ~finally:(fun () -> close_out_noerr channel)
        (fun () -> output_string channel (String.make (Centl_sci_mirage.max_document_bytes + 1) 'x'));
      match Centl_sci_mirage.ingest workspace document with
      | Ok _ -> Alcotest.fail "oversized document should be rejected"
      | Error message ->
          Alcotest.(check bool) "limit failure is explicit" true
            (Option.is_some
               (Centl_sci_interaction.find_substring ~needle:"admits at most" message)))
  ...
  Alcotest.run "CENTL-MIRAGE"
    [
      ( "specification ingestion",
        [
          Alcotest.test_case "cell classification" `Quick test_cell_classification;
          Alcotest.test_case "local cycle artifacts" `Quick
            test_ingestion_creates_local_cycle;
          Alcotest.test_case "document size limit" `Quick test_document_size_limit;
        ] );
    ]

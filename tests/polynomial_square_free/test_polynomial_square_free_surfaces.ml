...
let test_public_factorize () =
  ...
  let response =
    public_math ~id:(`String "sqf-public") state "polynomial_square_free"
      (factorize_request (repeated ()))
  in
  Alcotest.(check bool) "success" true (bool "ok" response);
  Alcotest.(check string) "id" "sqf-public" (string "id" response);
  ...
  Alcotest.(check string) "kind" "polynomial_square_free_factorization"
    (string "kind" result);
  Alcotest.(check string) "group semantics" "square_free_multiplicity_groups"
  ...
  Alcotest.(check int) "one group" 1 (int "factor_count" result);
  begin match list "factors" result with
  | [ factor ] ->
      Alcotest.(check int) "multiplicity" 2 (int "multiplicity" factor)
  | _ -> Alcotest.fail "expected one square-free multiplicity group"
  end
...
let test_public_capabilities () =
  ...
  in
  Alcotest.(check bool) "success" true (bool "ok" response);
  Alcotest.(check bool) "square-free domain advertised" true
...
let test_public_deep_cancellation () =
  ...
  in
  let response =
    public_math ~id:(`String "sqf-cancel") ~cancelled state
      "polynomial_square_free" (factorize_request (deep_repeated ()))
  in
  Alcotest.(check bool) "cancelled" false (bool "ok" response);
  Alcotest.(check string) "id" "sqf-cancel" (string "id" response);
  Alcotest.(check string) "domain" "polynomial_square_free"
    (string "domain" response);
...
let test_mcp_schema_and_factorization () =
  ...
  initialize state;
  let response = mcp_call state 2 (factorize_request (repeated ())) in
...
let test_mcp_nested_strictness () =
  ...
  let response =
    mcp_call state 2
      (factorize_request (repeated ()) @ [ ("irreducible", `Bool true) ])
  in
...
let test_mcp_deep_cancellation () =
  ...
  in
  let response =
    mcp_call ~cancelled state 2 (factorize_request (deep_repeated ()))
  in
  Alcotest.(check bool) "tool error" true (tool_is_error response);
  let protocol = structured response in
  Alcotest.(check string) "domain" "polynomial_square_free"

let admission_candidate state candidate_id transaction_fingerprint
...
let test_only_admissible_candidates_enter_review () =
  ...
  in
  let report =
    Centl_sci_mirage_review.prepare (admission [ admissible; pending; blocked ])
  in
  ...
  | [ candidate ] ->
      Alcotest.(check string)
        "candidate id" "candidate-a" candidate.candidate_id;
      Alcotest.(check int)
        "review fingerprint length" 64
...
let test_review_fingerprint_binds_evidence () =
  ...
  in
  let first =
    Centl_sci_mirage_review.prepare
      (admission [ candidate [ String.make 64 '1' ] ])
  in
  let second =
    Centl_sci_mirage_review.prepare
      (admission [ candidate [ String.make 64 '2' ] ])
  in
  match (first.candidates, second.candidates) with
  | [ left ], [ right ] ->
      Alcotest.(check bool)
        "different receipt evidence changes review identity" true
...
let test_construct_persists_review_manifest () =
  ...
  in
  let admission_path =
    Filename.temp_file "centl-mirage-review" ".admission.json"
  in
  ...
  Fun.protect
    ...
    (fun () ->
      match
        Centl_sci_mirage_review.construct admission_path
          (admission [ candidate ])
      with
      ...
      | Ok (path, _) ->
          ...
          let open Yojson.Safe.Util in
          Alcotest.(check string)
            "artifact kind" "candidate_review_manifest"

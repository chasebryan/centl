...
let action ?(supported = true) ?blocking_reason ~id ~kind ~executor
    ~precondition () : Centl_sci_mirage_execution_plan.action =
  {
    action_id = id;
...
let test_unimplemented_executor_stays_pending () =
  ...
  Fun.protect
    ...
    (fun () ->
      ...
      in
      Alcotest.(check bool)
        "pending evidence is not complete" false
        (Centl_sci_mirage_evidence.evidence_complete report);
      match report.receipts with
      | [ receipt ] ->
          Alcotest.(check string)
            "not fabricated as executed" "pending"
            (Centl_sci_mirage_evidence.receipt_state_text receipt.state)
      | _ -> Alcotest.fail "expected one evidence receipt")
          ...
          (plan
             [
               action ~supported:false
                 ~blocking_reason:"requires a human product decision"
                 ~id:"human-action" ~kind:"clarification_required"
                 ~executor:"human_resolution"
                 ~precondition:"blocking_requirement_resolved" ();
             ])
      in
      Alcotest.(check bool)
        "blocked evidence is not complete" false
        (Centl_sci_mirage_evidence.evidence_complete report);
      match report.receipts with
      | [ receipt ] ->
          Alcotest.(check string)
            "unsupported executor blocks" "blocked"
            (Centl_sci_mirage_evidence.receipt_state_text receipt.state)
      | _ -> Alcotest.fail "expected one evidence receipt")
  ...
  Fun.protect
    ...
    (fun () ->
      ...
      | Ok (path, _) ->
          ...
          Alcotest.(check bool)
            "named evidence cycle is complete" true
          ...
          Alcotest.(check int)
            "persisted fingerprint length" 64
            (receipt
            |> member "receipt_fingerprint"
            |> to_string |> String.length);
          Alcotest.(check bool)
            "candidate remains inactive" false
...
let () =
  Alcotest.run "CENTL-MIRAGE evidence execution"
    [
      ( "execution",
        [
          Alcotest.test_case "workspace snapshot" `Quick
            test_workspace_snapshot_executes;

...
let test_all_passed_is_admissible () =
  ...
  let receipt =
    Centl_sci_mirage_evidence.make_receipt action
      Centl_sci_mirage_evidence.Passed "deterministic regression gate passed"
      None
  in
  let candidate =
...
let test_pending_stays_pending () =
  ...
  let receipt =
    Centl_sci_mirage_evidence.make_receipt action
      Centl_sci_mirage_evidence.Pending "validator has not run" None
  in
  let candidate =
    Centl_sci_mirage_admission.assess (plan action []) (evidence receipt)
    |> only_candidate
...
let test_receipt_identity_mismatch_blocks () =
  ...
  let receipt =
    Centl_sci_mirage_evidence.make_receipt other
      Centl_sci_mirage_evidence.Passed "unrelated action passed" None
  in
  let candidate =
...
let test_receipt_contract_mismatch_blocks () =
  ...
  let receipt =
    Centl_sci_mirage_evidence.make_receipt action
      ...
      None
  in
  let altered = { receipt with executor = "unrelated_executor" } in
  let candidate =
...
let test_stale_receipt_fingerprint_blocks () =
  ...
  let altered =
    {
      receipt with
      evidence = "evidence changed after receipt identity was computed";
    }
  in
  let candidate =
    Centl_sci_mirage_admission.assess (plan action []) (evidence altered)
...
let test_construct_persists_transaction_bound_assessment () =
  ...
  in
  let evidence_path =
    Filename.temp_file "centl-mirage-admission" ".evidence.json"
  in
  ...
  Fun.protect
    ...
    (fun () ->
      ...
      | Ok (path, report) ->
          ...
          let open Yojson.Safe.Util in
          Alcotest.(check string)
            "artifact kind" "candidate_admission_assessment"
          ...
          Alcotest.(check string)
            "transaction fingerprint" (String.make 64 'a')
    ...
    [
      ( "admission",
        [
          ...
          Alcotest.test_case "source block" `Quick
            test_source_block_prevents_admission;

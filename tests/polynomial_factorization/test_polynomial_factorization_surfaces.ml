...
let factorize_request polynomial =
  [
...
let test_public_factorize () =
  ...
  let response =
    public_math ~id:(`String "factor-public") state "polynomial_factorization"
      (factorize_request (source ()))
  in
  Alcotest.(check bool) "success" true (bool "ok" response);
  Alcotest.(check string) "id" "factor-public" (string "id" response);
  Alcotest.(check string) "domain" "polynomial_factorization"
  ...
  | `List [ first; second ] ->
      ...
      Alcotest.(check string) "linear factor" "1/2 + x"
        (string "text" (assoc "polynomial" first));
      Alcotest.(check int) "quadratic multiplicity" 1
        (int "multiplicity" second);
      Alcotest.(check string) "quadratic factor" "1/3 + 1/3*x + x^2"
        (string "text" (assoc "polynomial" second))
  | _ -> Alcotest.fail "expected two canonical public factors"
  end
...
let test_public_capabilities () =
  ...
  in
  Alcotest.(check bool) "success" true (bool "ok" response);
  Alcotest.(check bool) "factorization advertised" true
    (json_contains_string "polynomial_factorization" (assoc "result" response))

let test_public_zero_refusal () =
  let state = Centl_protocol.create () in
  let response =
    public_math state "polynomial_factorization"
      (factorize_request (polynomial []))
  in
...
let test_public_result_limit () =
  ...
  let response =
    public_math state "polynomial_factorization" (factorize_request (source ()))
  in
...
let test_public_deep_cancellation () =
  ...
  in
  let response =
    public_math ~id:(`String "factor-cancel") ~cancelled state
      "polynomial_factorization" (factorize_request (deep_source ()))
  in
  Alcotest.(check bool) "cancelled" false (bool "ok" response);
  Alcotest.(check string) "id" "factor-cancel" (string "id" response);
  Alcotest.(check string) "domain" "polynomial_factorization"
    (string "domain" response);
  Alcotest.(check string) "cancel code" "cancelled"
    (string "code" (assoc "error" response));
  Alcotest.(check bool) "callback reached factorization core" true (!checks >= 20)
       ...
       (`Assoc
          [
            ...
            ( "clientInfo",
              `Assoc
                [
                  ("name", `String "polynomial-factorization-test");
                  ("version", `String "1");
                ] );
          ]));
  ignore
    ...
    (`Assoc
       [
         ...
         ( "arguments",
           `Assoc
             [
               ("domain", `String "polynomial_factorization");
...
let test_mcp_schema_and_factorization () =
  ...
  Alcotest.(check bool) "domain in closed schema" true
    (json_contains_string "polynomial_factorization" (assoc "inputSchema" tool));
  let state = Centl_mcp.create () in
  initialize state;
  let response = mcp_call state 2 (factorize_request (source ())) in
...
let test_mcp_nested_strictness () =
  ...
  let response =
    mcp_call state 2
      (factorize_request (source ()) @ [ ("partial", `Bool true) ])
  in
...
let test_mcp_zero_refusal () =
  let state = Centl_mcp.create () in
  initialize state;
  let response = mcp_call state 2 (factorize_request (polynomial [])) in
  Alcotest.(check bool) "tool error" true (tool_is_error response);
  Alcotest.(check string) "zero code" "zero_polynomial"
    (string "code" (assoc "error" (structured response)))
...
let test_mcp_deep_cancellation () =
  ...
  in
  let response =
    mcp_call ~cancelled state 2 (factorize_request (deep_source ()))
  in
  Alcotest.(check bool) "tool error" true (tool_is_error response);
  let protocol = structured response in
  Alcotest.(check string) "domain" "polynomial_factorization"
...
let () =
  Alcotest.run "centl rational polynomial factorization public surfaces"
    [
      ( "surfaces",
        [

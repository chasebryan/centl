...
let add_native workspace ~name ~source =
  ...
  match
    Centl_sci_workspace.write_manifest workspace ~name ~enabled:true
      ...
      ~source:("modules/" ^ name ^ ".centl") ~summary:"audit test"
  with
  | Ok _ -> ()
...
let test_healthy_is_structural_not_assurance () =
  ...
  Fun.protect
    ...
    (fun () ->
      ...
      Alcotest.(check bool) "not a trust score" true
        (Option.is_some
           (Centl_sci_interaction.find_substring ~needle:"not a trust score" rendered));
      Alcotest.(check bool) "local assurance remains visible" true
        (Option.is_some
...
let test_attention_required_for_invalid_extension () =
  ...
  Fun.protect
    ...
    (fun () ->
      ...
      let audit = Centl_sci_audit.collect workspace in
      Alcotest.(check string) "health" "attention_required"
        (Centl_sci_audit.health audit);
      Alcotest.(check int) "one invalid extension" 1
      ...
      | `Assoc fields ->
          ...
          with
          | Some (`String "attention_required"), Some (`Int 1), Some (`Bool false) -> ()

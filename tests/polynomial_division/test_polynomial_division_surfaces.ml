...
let dividend () = polynomial [ term "-1" []; term "1" [ power "x" 3 ] ]
let divisor () = polynomial [ term "-1" []; term "1" [ power "x" 1 ] ]

let division_request action =
  [
...
let test_public_divide () =
  let state = Centl_protocol.create () in
  let response =
    public_math ~id:(`String "divide-public") state "polynomial_division"
      (division_request "divide")
  in
  Alcotest.(check bool) "success" true (bool "ok" response);
  Alcotest.(check string) "id" "divide-public" (string "id" response);
  Alcotest.(check string) "domain" "polynomial_division"
...
let test_public_capabilities () =
  ...
  in
  Alcotest.(check bool) "success" true (bool "ok" response);
  Alcotest.(check bool) "division domain advertised" true
    (json_contains_string "polynomial_division" (assoc "result" response))

let test_public_zero_divisor () =
  let state = Centl_protocol.create () in
  let response =
    public_math state "polynomial_division"
      [
...
let test_public_result_limit () =
  ...
  let response =
    public_math state "polynomial_division" (division_request "divide")
  in
...
let test_public_deep_cancellation () =
  ...
  in
  let response =
    public_math ~id:(`String "divide-cancel") ~cancelled state
      "polynomial_division"
      [
        ("action", `String "divide");
        ("variable", `String "x");
        ("dividend", deep_dividend ());
        ("divisor", deep_divisor ());
      ]
  in
  Alcotest.(check bool) "cancelled" false (bool "ok" response);
  Alcotest.(check string) "id preserved" "divide-cancel" (string "id" response);
  Alcotest.(check string) "domain preserved" "polynomial_division"
    (string "domain" response);
  Alcotest.(check string) "cancel code" "cancelled"
    (string "code" (assoc "error" response));
  Alcotest.(check bool) "callback reached division core" true (!checks >= 14)
       ...
       (`Assoc
          [
            ...
            ( "clientInfo",
              `Assoc
                [
                  ("name", `String "polynomial-division-test");
                  ("version", `String "1");
                ] );
          ]));
  ignore
    ...
    (`Assoc
       [
         ...
         ( "arguments",
           `Assoc
             [
               ("domain", `String "polynomial_division");
...
let test_mcp_schema_and_divide () =
  let tool = Centl_math_mcp.tool () in
  Alcotest.(check bool) "division present in closed tool schema" true
    (json_contains_string "polynomial_division" (assoc "inputSchema" tool));
  let state = Centl_mcp.create () in
  initialize state;
  let response = mcp_call state 2 (division_request "divide") in
...
let test_mcp_nested_strictness () =
  ...
  let response =
    mcp_call state 2
      (division_request "divide" @ [ ("approximate", `Bool true) ])
  in
...
let test_mcp_zero_divisor () =
  ...
  in
  Alcotest.(check bool) "tool error" true (tool_is_error response);
  Alcotest.(check string) "division by zero" "division_by_zero"
    (string "code" (assoc "error" (structured response)))
...
let () =
  Alcotest.run "centl polynomial division public surfaces"
    [
      ( "surfaces",
        [
          Alcotest.test_case "JSONL divide" `Quick test_public_divide;
          Alcotest.test_case "JSONL capabilities" `Quick test_public_capabilities;
          Alcotest.test_case "JSONL zero divisor" `Quick test_public_zero_divisor;
          ...
          Alcotest.test_case "MCP schema and divide" `Quick
            test_mcp_schema_and_divide;
          Alcotest.test_case "MCP nested strictness" `Quick
            test_mcp_nested_strictness;
          Alcotest.test_case "MCP zero divisor" `Quick test_mcp_zero_divisor;

...
let sphere_body ?(radius = "1") ~id ~position ~velocity =
  sphere ~particle:(body ~id ~position ~velocity)
...
let test_rational_crossing_time () =
  let a =
    sphere_body ~id:"a" ~position:("0", "0", "0")
      ~velocity:("1", "0", "0")
  in
  let b =
    sphere_body ~id:"b" ~position:("4", "0", "0")
      ~velocity:("0", "0", "0")
  in
  let certificate = certify_linear_sphere_contact ~duration:(duration "3") a b in
  ...
  check_q "closest time is interval end" (q "3") certificate.closest_time.si_value;
...
let test_exact_tangent_contact () =
  ...
  in
  let b =
    sphere_body ~id:"b" ~position:("2", "2", "0")
      ~velocity:("0", "0", "0")
  in
  let certificate = certify_linear_sphere_contact ~duration:(duration "3") a b in
...
let test_irrational_quadratic_contact_time () =
  ...
  in
  let b =
    sphere_body ~id:"b" ~position:("3", "1", "0")
      ~velocity:("0", "0", "0")
  in
  let certificate = certify_linear_sphere_contact ~duration:(duration "3") a b in
  check_status "crossing" "crossing_contact" certificate;
...
let test_short_interval_certifies_no_contact () =
  ...
  let b =
    sphere_body ~id:"b" ~position:("4", "0", "0")
      ~velocity:("0", "0", "0")
  in
  let certificate = certify_linear_sphere_contact ~duration:(duration "1") a b in
  check_status "no contact" "no_contact_in_interval" certificate;
  check_q "closest sampled interval endpoint" Q.one certificate.closest_time.si_value;
  check_q "positive clearance certificate" (q "5")
    certificate.minimum_clearance_squared.si_value;
  Alcotest.(check bool)
    "no contact time" true (certificate.first_contact_time = None)
...
let test_parallel_miss_certifies_no_contact () =
  ...
  in
  let b =
    sphere_body ~id:"b" ~position:("0", "3", "0")
      ~velocity:("0", "0", "0")
  in
  let certificate = certify_linear_sphere_contact ~duration:(duration "10") a b in
...
let test_stationary_separation () =
  ...
  in
  let b =
    sphere_body ~id:"b" ~position:("3", "0", "0")
      ~velocity:("0", "0", "0")
  in
  let certificate = certify_linear_sphere_contact ~duration:(duration "10") a b in
  check_status "stationary separated" "no_contact_in_interval" certificate;
  Alcotest.(check bool) "no discriminant needed" true (certificate.discriminant = None)
...
let test_touching_at_start () =
  ...
  in
  let b =
    sphere_body ~id:"b" ~position:("2", "0", "0")
      ~velocity:("1", "0", "0")
  in
  let certificate = certify_linear_sphere_contact ~duration:(duration "5") a b in
  check_status "touching start" "touching_at_start" certificate;
  begin match certificate.first_contact_time with
  | Some (Rational_contact_time time) -> check_q "contact at zero" Q.zero time.si_value
...
let test_initial_overlap () =
  ...
  in
  let b =
    sphere_body ~id:"b" ~position:("1", "0", "0")
      ~velocity:("0", "0", "0")
  in
  let certificate = certify_linear_sphere_contact ~duration:(duration "5") a b in
  check_status "overlap" "initially_overlapping" certificate;
  check_q "negative initial clearance" (q "-3") certificate.polynomial_c.si_value
...
let test_negative_duration_rejected () =
  ...
  | exception Physics_error message -> Alcotest.fail ("unexpected error: " ^ message)
  ...
  Alcotest.run "centl certified linear contact"
    [
      ( "continuous linear contact",
        [
          Alcotest.test_case "rational crossing" `Quick test_rational_crossing_time;
          ...
          Alcotest.test_case "parallel miss" `Quick test_parallel_miss_certifies_no_contact;
          Alcotest.test_case "stationary separated" `Quick test_stationary_separation;

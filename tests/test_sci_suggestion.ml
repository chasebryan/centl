...
let test_constant_structural_hint_is_not_committed () =
  ...
  in
  Alcotest.(check bool) "structural" true
    (suggestion.category = Centl_sci_suggestion.Structural);
  Alcotest.(check bool) "not auto-accepted" false suggestion.safe_to_accept;
  Alcotest.(check bool) "catalog appears" true
    (contains "k_B" suggestion.display_text && contains "g0" suggestion.display_text);
  Alcotest.(check bool) "alternatives recorded" true
...
let test_approximation_structural_hint () =
  ...
  Alcotest.(check bool) "digit target" true
...
let test_verification_needs_relation () =
  ...
  in
  Alcotest.(check bool) "claim relation missing" true
...
let test_build_validation_needs_name () =
  ...
  in
  Alcotest.(check bool) "name missing" true
...
let test_mechanics_lists_missing_slots () =
  ...
  in
  Alcotest.(check bool) "position missing" true
    (List.mem "position" suggestion.missing_slots);
  Alcotest.(check bool) "gravity missing" true

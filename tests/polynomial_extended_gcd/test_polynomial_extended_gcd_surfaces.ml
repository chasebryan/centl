...
let test_public_extended_gcd () =
  let state = Centl_protocol.create () in
  let response =
    public_math ~id:(`String "extended-public") state
      "polynomial_extended_gcd" (extended_request ())
  in
  Alcotest.(check bool) "success" true (bool "ok" response);
  Alcotest.(check string) "id" "extended-public" (string "id" response);
  Alcotest.(check string) "domain" "polynomial_extended_gcd"
...
let test_public_capabilities () =
  ...
  in
  Alcotest.(check bool) "success" true (bool "ok" response);
  Alcotest.(check bool) "extended gcd advertised" true
    (json_contains_string "polynomial_extended_gcd" (assoc "result" response))

let test_public_mixed_variable () =
  let state = Centl_protocol.create () in
  let response =
    public_math state "polynomial_extended_gcd"
      [
        ("action", `String "extended_gcd");
        ("variable", `String "x");
        ("left", left ());
...
let test_public_result_limit () =
  ...
  let response =
    public_math state "polynomial_extended_gcd" (extended_request ())
  in
...
let test_public_deep_cancellation () =
  ...
  in
  let response =
    public_math ~id:(`String "extended-cancel") ~cancelled state
  ...
  Alcotest.(check string) "id" "extended-cancel" (string "id" response);
  Alcotest.(check string) "domain" "polynomial_extended_gcd"
    (string "domain" response);
  Alcotest.(check string) "cancel code" "cancelled"
    (string "code" (assoc "error" response));
  Alcotest.(check bool) "callback reached certificate core" true (!checks >= 24)
       ...
       (`Assoc
          [
            ...
            ( "clientInfo",
              `Assoc
                [
                  ("name", `String "polynomial-extended-gcd-test");
                  ("version", `String "1");
                ] );
          ]));
  ignore
    ...
    (`Assoc
       [
         ...
         ( "arguments",
           `Assoc
             [
               ("domain", `String "polynomial_extended_gcd");
...
let test_mcp_schema_and_certificate () =
  let tool = Centl_math_mcp.tool () in
  Alcotest.(check bool) "domain in closed schema" true
    (json_contains_string "polynomial_extended_gcd" (assoc "inputSchema" tool));
  let state = Centl_mcp.create () in
  initialize state;
  let response = mcp_call state 2 (extended_request ()) in
  ...
  Alcotest.(check int) "gcd terms" 2 (int "term_count" (assoc "gcd" result));
  Alcotest.(check string) "left witness" "multivariate_rational_polynomial"
    (string "kind" (assoc "left_coefficient" result))
...
let test_mcp_nested_strictness () =
  ...
  initialize state;
  let response =
    mcp_call state 2 (extended_request () @ [ ("monic", `Bool false) ])
  in
...
let () =
  Alcotest.run "centl polynomial extended gcd public surfaces"
    [
      ( "surfaces",
        [
          Alcotest.test_case "JSONL extended gcd" `Quick test_public_extended_gcd;
          ...
          Alcotest.test_case "MCP schema and certificate" `Quick
            test_mcp_schema_and_certificate;

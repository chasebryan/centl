...
let test_public_gcd () =
  ...
  let response =
    public_math ~id:(`String "gcd-public") state "polynomial_gcd"
      (gcd_request "gcd")
  in
  Alcotest.(check bool) "success" true (bool "ok" response);
  Alcotest.(check string) "id" "gcd-public" (string "id" response);
  Alcotest.(check string) "domain" "polynomial_gcd" (string "domain" response);
...
let test_public_coprime () =
  let state = Centl_protocol.create () in
  let response =
    public_math state "polynomial_gcd"
      [
  ...
  in
  Alcotest.(check bool) "success" true (bool "ok" response);
  Alcotest.(check bool) "coprime" true (bool "coprime" (assoc "result" response))
...
let test_public_capabilities () =
  ...
  in
  Alcotest.(check bool) "success" true (bool "ok" response);
  Alcotest.(check bool) "gcd domain advertised" true
    (json_contains_string "polynomial_gcd" (assoc "result" response))
...
let test_public_result_limit () =
  ...
  let response = public_math state "polynomial_gcd" (gcd_request "gcd") in
...
let test_public_deep_cancellation () =
  ...
  in
  let response =
    public_math ~id:(`String "gcd-cancel") ~cancelled state "polynomial_gcd"
      [
        ("action", `String "gcd");
        ("variable", `String "x");
        ("left", deep_left ());
  ...
  Alcotest.(check string) "id preserved" "gcd-cancel" (string "id" response);
  Alcotest.(check string) "domain preserved" "polynomial_gcd"
    (string "domain" response);
  Alcotest.(check string) "cancel code" "cancelled"
    (string "code" (assoc "error" response));
  Alcotest.(check bool) "callback reached deep gcd work" true (!checks >= 20)
       ...
       (`Assoc
          [
            ...
            ( "clientInfo",
              `Assoc
                [
                  ("name", `String "polynomial-gcd-test");
                  ("version", `String "1");
                ] );
          ]));
  ignore
...
let test_mcp_schema_and_gcd () =
  let tool = Centl_math_mcp.tool () in
  Alcotest.(check bool) "gcd present in closed tool schema" true
    (json_contains_string "polynomial_gcd" (assoc "inputSchema" tool));
  let state = Centl_mcp.create () in
  initialize state;
  let response = mcp_call state 2 (gcd_request "gcd") in
  ...
  Alcotest.(check string) "domain" "polynomial_gcd" (string "domain" protocol);
  Alcotest.(check int) "x-1 terms" 2 (int "term_count" (assoc "result" protocol))

let test_mcp_coprime () =
  let state = Centl_mcp.create () in
  initialize state;
  let response =
    mcp_call state 2
  ...
  in
  Alcotest.(check bool) "tool success" false (tool_is_error response);
  Alcotest.(check bool) "coprime" true
    (bool "coprime" (assoc "result" (structured response)))

let test_mcp_nested_strictness () =
  let state = Centl_mcp.create () in
  initialize state;
  let response = mcp_call state 2 (gcd_request "gcd" @ [ ("monic", `Bool false) ]) in
...
let test_mcp_result_limit_parity () =
  let evaluation =
    { Centl_protocol.default_server_limits.evaluation with max_result_bytes = 256 }
  in
  let limits = { Centl_protocol.default_server_limits with evaluation } in
...
let () =
  Alcotest.run "centl polynomial gcd public surfaces"
    [
      ( "surfaces",
        [
          Alcotest.test_case "JSONL gcd" `Quick test_public_gcd;
          Alcotest.test_case "JSONL coprime" `Quick test_public_coprime;
          ...
          Alcotest.test_case "MCP schema and gcd" `Quick test_mcp_schema_and_gcd;
          Alcotest.test_case "MCP coprime" `Quick test_mcp_coprime;
